...
TINY.table=function(){
	...
	sorter.prototype.init=function(){
		...
		if(this.p.colddid){
			d=T$(this.p.colddid); while(d.hasChildNodes()){d.removeChild(d.firstChild)}
		...
		for(i;i<t.w;i++){
			...
			if(d){
				var o=document.createElement('option'); o.value=i; var h=T$$('h3',c)[0]; o.innerHTML=(h?h:c).innerHTML; d.appendChild(o)
			}
		}
		this.reset()
	};
	...
	sorter.prototype.page=function(s){
		...
		if(this.p.pageddid){T$(this.p.pageddid).selectedIndex=this.g-1}
	};
	sorter.prototype.move=function(d,m){
		this.goto(d==1?(m?this.d:parseInt(this.g)+1):(m?1:parseInt(this.g)-1))
	};
...
//			d.setAttribute('onchange',this.n+'.goto(this.options[this.selectedIndex].value)');
