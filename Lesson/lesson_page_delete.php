<?php
	...
	//消す前に必要なＵＲＬを退避させる
	$select_sql = "SELECT div_url, page_num FROM board WHERE date='".$date."' AND subject_seq='".$subject_seq."'AND class_seq ='".$group_seq."'AND end_flg='0' AND page_num >".$page_num.";";
	$result = mysql_query($select_sql);
	$count = mysql_num_rows($result);

	//削除ボタンをおしたpage_num以降のデータをいったん削除
	$delete_sql = "DELETE FROM board WHERE date='".$date."' AND subject_seq='".$subject_seq."'AND class_seq='".$group_seq."' AND end_flg='0'AND page_num >=".$page_num.";";
	...
	for($i= 0;$i<$count;$i++){
		...
		$sql = "INSERT INTO board VALUE(0,'".$date ."', '".$group_seq."','".$subject_seq ."','".$new_page_num ."','".$row['div_url']."','0','0');";
